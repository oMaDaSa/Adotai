...
export function AdopterDashboard({ onBack, onStartChat, user }: AdopterDashboardProps) {
  ...
  const stats = {
    ...
    completedAdoptions: adoptionRequests.filter(r => r.status === 'completed').length,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-600">Adoções Concluídas</p>
                  <p className="text-2xl font-bold text-green-600">{stats.completedAdoptions}</p>
                </div>
                <HeartHandshake className="h-8 w-8 text-green-500" />
              </div>
            </CardContent>
          </Card>
        </div>
                          ...
                          <div className="flex-1">
                            ...
                            {request.status_message && (
                              <div className={`p-3 rounded-lg mt-3 ${
                                ...
                                request.status === 'denied' ? 'bg-red-50 border border-red-200' :
                                request.status === 'completed' ? 'bg-blue-50 border border-blue-200' :
